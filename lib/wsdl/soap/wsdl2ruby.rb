...
require 'wsdl/soap/workatoSchemaCreator'
...
private
  ...
  def create_file
    ...
    puts "CREATE FILE"
    create_schema if @opt.key?('schemadef')
  ...
  def create_schema
    @logger.info { "Creating workato schema definition." }
    @mr_filename = @name + '_workato_schema' + '.rb'
    puts "CHECK FILE"
    ...
    write_file(@mr_filename) do |f|
      ...
      f << WSDL::SOAP::WorkatoSchemaCreator.new(@wsdl, @name_creator, @modulepath, @ns_version, {}).dump
    end
  end
